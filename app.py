...
sorted_df = amenity_gdf_indexed.sort_values(by='college')
bar_fig = px.bar(sorted_df, x='college', y=sorted_df.index, 
                 color_continuous_scale='teal', color='college', template='plotly_white')
...
navbar = dbc.NavbarSimple(
    ...
    brand="PH Educational Institutions by Cara Del Rosario",
    brand_href="#",
    color="primary",
    dark=True,
)
...
app.layout = html.Div(children=[
    ...
    dbc.Container(children=[
        dbc.Row(children=[
            ...
            html.P("A dashboard visualizing educational institutions across the Philippines"),
            html.P("Use the dropdown menu to view visualization options.")
        ]),
        dbc.Row(children=[
        ...
        dbc.Row(children=[
            dbc.Col(children=[
                dcc.Loading(id="map-loading", children=dcc.Graph(id="map-graph", figure=fig ))
            ], width=6),
            dbc.Col(children=[
                dcc.Loading(id="bar-loading", children=dcc.Graph(id="bar-list", figure=bar_fig))
            ], width=6)
        ])
    ])
])
...
def display_map(selected_value, selected_type):
    ...
    bar_fig = px.bar(sorted_df, x=selected_value, y=sorted_df.index, 
                 color_continuous_scale='teal', color=selected_value, template='plotly_white',height=800)
